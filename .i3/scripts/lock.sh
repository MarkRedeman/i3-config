...
icon="$HOME/.i3/assets/lock-icon.png"
...
local __reduced_x_2=$(_calc "${__reduced_x} * 2")
local __reduced_y_2=$(_calc "${__reduced_y} * 2")
...
i3lock $* -u -i <(import -screen -window root -silent png:- \
        | convert png:- -scale '!'"${__reduced_x}x${__reduced_y}" -scale '!'"${__screen_x}x${__screen_y}" png:- \
        | convert png:- "$icon" -gravity center -composite -matte png:- \
        # | convert png:- -scale '!'"${__reduced_x_2}x${__reduced_y_2}" -scale '!'"${__screen_x}x${__screen_y}" png:- \
        )